...
X_ENC  = 228
X_FORK = 326
X_GATE = 464;  Y_GATE = 100
X_ADPT = 634
Y_A    = [162, 264, 366, 468]   # adapter centres (3-4, 5-7, 8-11, unique)
X_BUS  = 764          # weight bus x  (also centre of ⊕ nodes)
...
def sum_node(svg, cx, cy, r=SUM_R, stroke=C["line"]):
    """⊕ drawn as circle + crosshairs (no glyph)."""
    _circle(svg, cx, cy, r, fill=C["wt"], stroke=stroke, sw=1.6)
    # horizontal bar
    svg.e(f'<line x1="{cx-r+3:.1f}" y1="{cy:.1f}" x2="{cx+r-3:.1f}" y2="{cy:.1f}" '
          f'stroke="{stroke}" stroke-width="1.5"/>')
    # vertical bar
    svg.e(f'<line x1="{cx:.1f}" y1="{cy-r+3:.1f}" x2="{cx:.1f}" y2="{cy+r-3:.1f}" '
          f'stroke="{stroke}" stroke-width="1.5"/>')
...
def draw(out_stem: Path):
    ...
    _text(svg, X_IN, mel_y - 12, "Speech Audio", sz=10, fill=C["io"], bold=True)
    _text(svg, X_IN, mel_y + mel_h + 13, "Mel Spectrogram", sz=8, fill=C["gray"], italic=True)
    ...
    _path(svg, f"M {X_IN+32:.0f},{Y_SP} L {X_ENC-ENC_HW:.0f},{Y_SP}",
          color=C["line"], w=1.8, marker="a_dk")
    _text(svg, (X_IN+32 + X_ENC-ENC_HW)//2, Y_SP - 7,
          "waveform", sz=8, fill=C["gray"], italic=True)

    # Fork dot
    _line(svg, X_ENC+ENC_HW, Y_SP, X_FORK, Y_SP, color=C["line"], w=1.8)
    _circle(svg, X_FORK, Y_SP, 6, fill=C["line"], stroke=C["line"])
    ...
    # Dims BELOW
    _text(svg, X_GATE, gate_bottom + 15, "768 &#x2192; 512 &#x2192; 3",
          sz=9.5, fill=C["dt"])
    _text(svg, X_GATE, gate_bottom + 28, "+ Temperature Scaling",
          sz=8.5, fill=C["gray"], italic=True)

    # Fork → Gate: right-angle up then right, arrives at gate left edge
    _path(svg,
          f"M {X_FORK},{Y_SP} L {X_FORK},{Y_GATE} L {gate_left},{Y_GATE}",
          color=C["gate"], w=1.6, marker="a_gate")
    _text(svg, X_FORK - 28, (Y_SP + Y_GATE)//2 - 6, "pooled",
          sz=8, fill=C["gate"], italic=True, anchor="middle")
    _text(svg, X_FORK - 28, (Y_SP + Y_GATE)//2 + 8, "embeds",
          sz=8, fill=C["gate"], italic=True, anchor="middle")

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # (d) Input feature bus → adapters
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

    adpt_left  = X_ADPT - ADPT_HW
    adpt_right = X_ADPT + ADPT_HW
    feat_bus_x = adpt_left - 14

    _line(svg, X_FORK, Y_SP, feat_bus_x, Y_SP, color=C["line"], w=1.8)
    _line(svg, feat_bus_x, Y_A[0] - ADPT_HH - 6,
               feat_bus_x, Y_A[3] + ADPT_HH + 6,
          color=C["lgray"], w=1.4)
    ...
    adapter_defs = [
        ...
        (Y_A[3], "LoRA  (all ages)", "unique subjects split",          C["uniq"], "a_uniq", None),
    ]

    for ya, name, sub, col, arr, _ in adapter_defs:
        _path(svg, f"M {feat_bus_x:.0f},{ya:.0f} L {adpt_left:.0f},{ya:.0f}",
              color=C["lgray"], w=1.4, marker=arr)
        adapter_box(svg, X_ADPT, ya, ADPT_HW, ADPT_HH, name, sub, col)

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # (f) MoLE Ensemble box (dashed) — wraps adapters + ⊕ + Σ column
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

    ens_pad = 18
    ens_x = X_BUS - 2 * ens_pad # feat_bus_x - ens_pad
    ens_y = Y_A[0] - ADPT_HH - ens_pad
    ens_w = (X_SIG + SIGMA_R) - ens_x + ens_pad
    ens_h = (Y_A[3] + ADPT_HH) - ens_y + ens_pad

    svg.e(
        f'<rect x="{ens_x:.0f}" y="{ens_y:.0f}" '
        f'width="{ens_w:.0f}" height="{ens_h:.0f}" '
        f'rx="12" fill="{C["ens_bg"]}" stroke="{C["ens"]}" '
        f'stroke-width="1.6" stroke-dasharray="8,4" opacity="0.9"/>'
    )
    # Label top-right inside box
    _text(svg, ens_x + ens_w - 10, ens_y + 16,
          "MoLE Ensemble", sz=10, fill=C["ens"], bold=True, anchor="end")


    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # (e) Weight bus: gate right → horizontal at Y_GATE → vertical → ⊕ nodes
    #     Bus stays RIGHT of adapters, ABOVE all adapter boxes
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    # Vertical bus down to Y_A[2]
    _line(svg, X_BUS, Y_GATE, X_BUS, Y_A[2],
          color=C["gate"], w=1.5, dashed=True)

    # "routing weights" label
    _text(svg, X_BUS + 8, (Y_GATE + Y_A[0])//2 - 4,
          "routing", sz=8, fill=C["gate"], italic=True, anchor="start")
    _text(svg, X_BUS + 8, (Y_GATE + Y_A[0])//2 + 9,
    ...
    _text(svg, X_ADPT, ens_y - 1,
          "(parallel, frozen base encoder)", sz=8, fill=C["gray"], italic=True)

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # (h) ⊕ nodes + weight ticks
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

    weight_colours = [C["l0"], C["l1"], C["l2"]]
    weight_labels  = ["w&#x2081;", "w&#x2082;", "w&#x2083;"]

    for i in range(3):
        ya = Y_A[i]
        _line(svg, X_BUS - 10, ya, X_BUS, ya, color=C["gate"], w=1.2)
        _text(svg, X_BUS - 13, ya + 5, weight_labels[i],
              sz=9.5, fill=weight_colours[i], bold=True, anchor="end")

    for i, (ya, _, _, col, arr, wl) in enumerate(adapter_defs):
        if wl is not None:
            _path(svg, f"M {adpt_right},{ya:.0f} L {X_BUS - SUM_R:.0f},{ya:.0f}",
                  color=col, w=1.6, marker=arr)
            sum_node(svg, X_BUS, ya, r=SUM_R, stroke=col)
            _path(svg, f"M {X_BUS + SUM_R},{ya:.0f} L {X_SIG},{ya:.0f}",
                  color=col, w=1.4, marker=arr)
        else:
            _path(svg, f"M {adpt_right},{ya:.0f} L {X_SIG},{ya:.0f}",
                  color=col, w=1.6, marker=arr)
    ...
    _text(svg, X_SIG, eq_y + 14,
          "weighted combination", sz=8, fill=C["gray"], italic=True)

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # (j) Σ node — vertical bus then node touching decoder
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

    _line(svg, X_SIG, Y_A[0], X_SIG, Y_A[3], color=C["lgray"], w=1.2)
    ...
    for label, x in [
        ("(a) Feature extraction",   X_ENC),
        ("(b) Age routing",          X_GATE),
        ("(c) Parallel adaptation",  X_ADPT),
        ("(d) Weighted combination", X_SIG),
        ("(e) Transcription",        X_DEC),
    ]:
        _text(svg, x, foot_y - 12, label, sz=8.5, fill=C["gray"], italic=True)

    fn_cx = (line_x0 + line_x1) / 2
    _text(svg, fn_cx, foot_y + 8,
          "&#x2020; LoRA (all ages) = unique-subjects adapter; "
          "not routed by gating network &#x2014; standalone ASR adapter.",
          sz=8.5, fill=C["gray"], italic=True)
