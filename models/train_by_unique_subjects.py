"""
train_by_unique_subjects.py — Whisper-small + LoRA adapter for unique child IDs
================================================================================
Trains a single LoRA adapter across all child IDs (speaker-independent split).
...
class UniqueSubjectsAdapter:
    """Trains a single LoRA adapter across all child IDs (speaker-independent)."""

    def __init__(self, config: Config, base_model, processor: WhisperProcessor,
    ...
    def train(self, train_json: str, val_json: str) -> None:
        """Train the adapter and write the best checkpoint to weights/best/unique_subjects/.
    ...
    def _build_lora_model(self):
        """Construct a fresh LoRA-wrapped Whisper model from the snapshotted weights."""
